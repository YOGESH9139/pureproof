...
import { issueKYC } from '@/utils/kycApi';
import algosdk from 'algosdk';

export default function ScannerPage() {
  const router = useRouter();
  const webcamRef = useRef<Webcam>(null);
  const { walletAddress, peraWallet, setKycStatus, addDocument } = useAppStore();
  ...
  const loadingMessages = [
    ...
    "Awaiting Payment & Verifying on Algorand Testnet..."
  ];
  ...
  }, [walletAddress, peraWallet, fullName, idNumber, docType]);

  const completeVerification = async () => {
    try {
      if (!walletAddress || !peraWallet) throw new Error("No wallet connected");

      // Custom signer wrapper for PeraWalletConnect
      const signTransactions = async (txns: Uint8Array[]) => {
        const decodedTxns = txns.map(t => algosdk.decodeUnsignedTransaction(t));
        const txnsToSign = decodedTxns.map(t => ({ txn: t, signers: [walletAddress] }));
        return await peraWallet.signTransaction([txnsToSign]);
      };
      
      const signer = { address: walletAddress, signTransactions };
      ...
      // Calls x402 issue endpoint!
      await issueKYC(signer, dummyProof, publicSignals, mockPassportData, 'BASIC');
